...
class DenseRetriever(BaseRetriever):
    ...
    def _batch_search(
        self, query_list: List[List[str]], num: int = None, return_score=False
    ):
        ...
        for start_idx in tqdm(
            ...
            # 目前query_batch是一个batch_size长的list，每个元素也是一个list，每个list里有若干subquery
            batch_embs = [self.encoder.encode(sub_query_batch) for sub_query_batch in query_batch]
            # 分别对每个query的所有subquery进行embedding
            batch_scores_idxs = [self.index.search(batch_emb, k=num) for batch_emb in batch_embs]
            # print(batch_idxs)
            batch_scores = [score_idx[0].tolist() for score_idx in batch_scores_idxs]
            batch_idxs = [score_idx[1].tolist() for score_idx in batch_scores_idxs]
            # batch_scores = batch_scores.tolist()
            # batch_idxs = batch_idxs.tolist()
            # flat_idxs = [sum(batch_idx, []) for batch_idx in batch_idxs]
            batch_results = [[load_docs(self.corpus, idx) for idx in batch_idx] for batch_idx in batch_idxs]
